...
export async function createItineraryRecord(o: { templateId?: string; bookingId?: string; name?: string; userId: string }) {
  let content: ItineraryContent = blankItinerary(); let name = (o.name ?? "").trim() || "New itinerary"; let sourceTemplateId: string | null = null;
  if (o.templateId) { const [t] = await db.select().from(s.itineraries).where(eq(s.itineraries.id, o.templateId)); if (t) { content = reid(parseJson<ItineraryContent>(t.content, content)); sourceTemplateId = t.id; if (!o.name?.trim()) name = t.name; } }
  if (o.bookingId) {
  ...
  const [it] = await db.insert(s.itineraries).values({ name, content: JSON.stringify(content), bookingId: o.bookingId || null, sourceTemplateId, createdById: o.userId }).returning();
  return it;
}
