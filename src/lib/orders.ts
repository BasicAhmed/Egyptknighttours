...
export type OrderRow = { id: string; ref: string; status: string; title: string; name: string; email: string; whatsapp: string; country: string; travelDate: string; pax: number; isPrivate: boolean; total: number; paid: number; currency: string; createdAt: number; invoices: number; invoiceSent: number; itineraries: number; itinerarySent: number; hotel: string };
...
export async function loadOrder(id: string): Promise<Order | null> {
  const [main, payments, docs, events, docEvents, its, templates, methods, g] = await Promise.all([
