...
const tone = (st: string) => (st === "PUBLISHED" ? "bg-[#DFF3E6] text-[#17663A]" : st === "DRAFT" ? "bg-gold-500/25 text-[#6B4A0C]" : "bg-ink/10 text-ink/65");
    ...
    <div>
      ...
      <ul className="mt-5 space-y-2.5">{rows.map((t) => (
        <li key={t.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-ink/10 bg-white p-4">
          <div className="min-w-0"><p className="truncate font-display text-[17px] font-extrabold">{t.title}</p><p className="text-sm text-ink/65">{money(t.discountPrice ?? t.price)} {t.pricingModel === "PER_GROUP" ? "per group" : "per person"} · {t.popularity} bookings</p></div>
