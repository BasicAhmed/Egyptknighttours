...
export default async function AdminDestinations({ searchParams }: { searchParams: Promise<{ saved?: string }> }) {
  ...
  return (
    <div>
      ...
      <p className="mt-4 text-sm text-ink/55">The photo you upload here appears on the homepage, the destination page, and on any tour in that destination that has no photo of its own.</p>
      <ul className="mt-4 grid gap-3 sm:grid-cols-2">{rows.map((d) => (
        <li key={d.id} className="flex items-center gap-4 rounded-2xl border border-ink/10 bg-white p-3">
          ...
          <div className="min-w-0 flex-1"><p className="font-display text-lg font-extrabold">{d.name}</p><p className="truncate text-sm text-ink/55">{d.imageUrl ? "Custom photo" : "Illustration (no photo yet)"}</p></div>
