...
export default async function AdminHome({ searchParams }: { searchParams: Promise<{ open?: string; denied?: string }> }) {
  ...
  return <><AutoRefresh />{sp.denied && <p className="mb-4 rounded-xl bg-red-50 p-3 text-sm font-medium text-red-800">Your role doesn't have access to that section.</p>}<OrdersBoard initial={rows} tours={tours} openId={sp.open} /></>;
}
