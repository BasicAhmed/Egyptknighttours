...
export async function GET(req: Request, { params }: { params: Promise<{ bookingId: string }> }) {
  ...
  return new NextResponse(new Uint8Array(await renderInvoice(data)), { headers: { "Content-Type": "application/pdf", "Content-Disposition": `inline; filename="preview-${data.ref}.pdf"`, "Cache-Control": "no-store" } });
}
