...
export default function ItineraryEditor({ id, isTemplate, status, initial, bookings, docs, currency = "USD" }: { id: string; isTemplate: boolean; status: string; initial: Init; bookings: { id: string; label: string }[]; docs: { id: string; number: string; sent: string | null; created: string }[]; currency?: string }) {
  ...
  const priced = cost !== "" && margin !== "" && Number(cost) >= 0 && Number(margin) >= 0; const calc = priced ? Math.round(Number(cost) * (1 + Number(margin) / 100) * 100) / 100 : null;
    ...
    <div className="space-y-5 pb-24">
      ...
      <section className="card p-4">
        ...
        {open.trip && <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <h2 className="font-display text-lg font-bold sm:col-span-2">Closing page (drives the booking)</h2>
          <div className="sm:col-span-2 rounded-xl border border-gold-600/40 bg-gold-500/10 p-4">
            <p className="label mb-2">Price settings — this trip's own cost and profit margin</p>
            <div className="grid gap-3 sm:grid-cols-3">
              <div><label className="label" htmlFor="ip-cost">Cost of this trip ({currency}, no profit)</label><input id="ip-cost" type="number" min={0} step="any" className="input !py-2" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="hotel, guide, driver, entrance fees" /></div>
      ...
      {c.days.map((d, i) => (
        <section key={d.id} className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <button type="button" onClick={() => setOpen({ ...open, [d.id]: !(open[d.id] ?? i < 2) })} className="text-left"><span className="font-display text-2xl font-extrabold text-ink">Day {i + 1}</span><span className="ml-3 font-semibold">{d.title || "Untitled day"}</span><span className="ml-2 text-sm text-ink/65">{d.location}</span></button>
