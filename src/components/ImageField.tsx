    ...
    <div className="min-w-0">
      ...
      <div className={`flex items-center gap-3 rounded-xl border border-ink/15 bg-white p-2.5 ${compact ? "" : "sm:p-3"}`}>
        <div className={`flex shrink-0 items-center justify-center overflow-hidden rounded-lg bg-ink/[.06] ${compact ? "h-14 w-20" : "h-20 w-28"}`}>
          {url ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={url.startsWith("/api/media/") ? `${url}?w=480` : url} alt="" className="h-full w-full object-cover" /> : <span className="px-2 text-center text-[11px] font-semibold text-ink/40">No photo</span>}
        </div>
