...
export default function LeadsBoard({ initial, canEdit }: { initial: LeadRow[]; canEdit: boolean }) {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-extrabold sm:text-3xl">Inquiries</h1><p className="text-sm text-ink/55">People who asked a question, built a trip or started checkout. Reply fast, then turn them into orders.</p>
      ...
      <ul className="mt-4 space-y-2.5">{list.map((r) => (
        <li key={r.id}><button onClick={() => setOpen(r.id)} className="grid w-full gap-1 rounded-2xl border border-ink/10 bg-white p-4 text-left transition hover:border-ink/40 md:grid-cols-[1.2fr_2fr_auto] md:items-center">
          <span className="min-w-0"><span className="block truncate font-display text-[17px] font-extrabold">{r.name}</span><span className="block truncate text-sm text-ink/55">{KIND[r.kind] ?? r.kind} · {ago(r.createdAt)}{r.country ? ` · ${r.country}` : ""}</span></span>
        ...
        {!list.length && <li className="rounded-2xl border border-dashed border-ink/20 bg-white p-10 text-center text-ink/55">No inquiries here.</li>}</ul>
...
function LeadModal({ r, canEdit, onClose, onSaved }: { r: LeadRow; canEdit: boolean; onClose: () => void; onSaved: (p: Partial<LeadRow>) => void }) {
  ...
  const Row = ({ k, v }: { k: string; v: string }) => v ? <div className="flex justify-between gap-4 py-1 text-sm"><span className="text-ink/55">{k}</span><span className="text-right font-medium">{v}</span></div> : null;
  return (
    <Modal onClose={onClose} title={r.name} subtitle={`${KIND[r.kind] ?? r.kind} · ${ago(r.createdAt)}`}>
      <div className="space-y-4">
        ...
        {r.message && <div className="rounded-2xl bg-ink/[.04] p-4 text-sm"><p className="mb-1 text-xs font-bold uppercase tracking-wide text-ink/50">Message</p><p className="whitespace-pre-line">{r.message}</p></div>}
